...
const UserDetails = (user) => {
  // {foo.bar && <someMarkUp /> allows you to conditionally render tags}
  return (
    <div>
      {!!user.score && <li className="list-group-item"><h3>Score: {user.score}</h3></li>}
      <li className="list-group-item"> <img src={user.info.avatar_url} className="img-rounded img-responsive" style={{margin: "0 auto"}}/></li>
...
const propTypes = {
  score: PropTypes.number, // not required
  info: PropTypes.shape({ // IE the `shape` of an object
